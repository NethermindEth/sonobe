...
pub fn get_test_z<F: PrimeField>(input: BigUint, POWER: usize) -> Vec<F> {
    let one = BigUint::one();
    let five = &one + &one + &one + &one + &one;
...
pub fn get_test_z_albert<F: PrimeField>(input: BigUint, POWER: usize) -> Vec<F> {
    // let one = BigUint::one();
    // let five = &one + &one + &one + &one + &one;
    // let size = 1 << POWER; // Calculate size only once
    // let mut z_vec = Vec::with_capacity(size); // Preallocate memory for // efficiency
...
fn create_large_diagonal_matrix_2<F: PrimeField>(POWER: usize) -> SparseMatrix<F> {
